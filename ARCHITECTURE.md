...
状态机不引用 Tkinter、串口或真实时间，因此测试可以注入虚拟时间，覆盖内部自动搜索策略、超时、条纹丢失和串口失联。电机插件只暴露手动命令，自动参数由配置和实验流程状态机管理。
...
千分尺的通信方式和数据格式尚未确定，目前只保留适配器和位移字段，不参与自动流程启动条件。后续接入读数后，动镜位移按“读数变化 × 机构放缩系数”计算。流程状态机不直接访问 Tkinter 或串口，因此阶段判断和停车条件可以无硬件测试。
